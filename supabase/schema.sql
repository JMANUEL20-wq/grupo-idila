...
create table public.cotizaciones (
  ...
  cliente text not null,
  nombre text not null default 'Cotización',
  telefono text,
  direccion text,
...
-- =========================================================
-- MIGRACIÓN (ejecuta esto si ya habías corrido este schema antes)
-- =========================================================
alter table public.cotizaciones add column if not exists nombre text not null default 'Cotización';
alter table public.cotizaciones add column if not exists telefono text;
alter table public.cotizaciones add column if not exists direccion text;
alter table public.cotizaciones add column if not exists items jsonb not null default '[]'::jsonb;

alter table public.finanzas drop constraint if exists finanzas_tipo_check;
alter table public.finanzas add constraint finanzas_tipo_check check (tipo in ('ingreso','gasto'));

alter table public.proyectos add column if not exists empresa text;
alter table public.proyectos add column if not exists correo text;
alter table public.proyectos add column if not exists tipo_trabajo text;
alter table public.proyectos add column if not exists fecha_entrega date;
alter table public.proyectos add column if not exists prioridad text default 'Normal';
alter table public.proyectos drop constraint if exists proyectos_prioridad_check;
alter table public.proyectos add constraint proyectos_prioridad_check check (prioridad in ('Baja','Normal','Alta','Urgente'));
